...
func New(handlers ...Handler) *Cli {
	...
	// in order to handle `docking help` appropriately
	cli := new(Cli)
...
func (cli *Cli) noSuchCommand(command string) {
	...
	fmt.Fprintf(cli.Stderr, "docking: '%s' is not a docking command.\nSee 'docking --help'.\n", command)
	os.Exit(1)
}

// CmdHelp displays information on a docking command.
//
// If more than one command is specified, information is only shown for the first command.
//
// Usage: docking help COMMAND or docking COMMAND --help
...
// Subcmd is a subcommand of the main "docking" command.
// A subcommand represents an action that can be performed
// from the docking command line client.
//
// To see all available subcommands, run "docking --help".
func Subcmd(name string, synopses []string, description string, exitOnError bool) *flag.FlagSet {
	...
	flags.ShortUsage = func() {
		...
		for i, synopsis := range synopses {
			...
			fmt.Fprintf(flags.Out(), "\n%sdocking %s%s%s", lead, name, options, synopsis)
		}
